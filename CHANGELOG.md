...
- `locku`: the settings screen — a saver's layout, time and date shapes, and its two colours by RGB sliders (a draft until `S` saves it, `R` drops it, `q` asks when one is unsaved); savers duplicated, renamed, deleted, previewed one by one with `p`; preference: the PIN (set; once set, the current one opens the choice of a new one or none), the active saver, `show_status`, `prompt_timeout`, the lockout. Everything but the colours is written at once. `P` previews the lock in place.
