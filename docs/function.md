...
| custom（2026-09-25） | `command`：使用者自己的指令，`sh -c` 跑，畫面由它畫；`bg` / `fg` 給 PIN 框與結束時的板子 | 不經畫布：程式在 locku 開的 pty 上跑，輸出原樣轉到終端機；程式結束（它不該結束）就換成 locku 的板子，`COMPLETED`（exit 0）或 `ERROR`（其他），見 5.5 | 無，由程式自己 |
...
- **按鍵 → PIN**：不停止程式，只是暫停轉發它的輸出（讀掉丟掉，pty 不會塞住），離開 alt screen、還原 termios，跑一個只有 PIN 框的 lock 程式（同一套 prompt、cooldown、timeout，底是 profile 的 bg）；Esc 或逾時就結束那個程式、重新 raw + alt screen、恢復轉發、送一個 SIGWINCH 讓它重畫（curses 程式都會整頁重畫）；PIN 對了殺程式、結束。沒設 PIN 時第一個鍵就結束。tty 消失：殺程式、結束、tmux 旗不清。
- **程式結束**（使用者 2026-09-25 定案）：鎖不退，畫面換成 locku 現在的點陣板格式，字用 clock 的 3x7 字型，large → medium → small 依尺寸退階，放不下就純文字；狀態列紅字寫原因。exit 0 → `COMPLETED`（跑完了，但不該跑完；狀態列 `custom saver exited 0`）；exit 非 0 → `ERROR`（`custom saver: exit 127 · sh: cmatrix: command not found`，stderr 最後一行）；被訊號殺 → `ERROR`（`killed: segmentation fault`）；沒填 command → `ERROR`（`no command`）。兩種都不重啟、PIN 照常。
...
profiles:
  ...
  - name: dino
    ...
    fg: "#f2b753"
  - name: matrix
    saver: custom         # custom（2026-09-25）：使用者自己的程式，見 5.5
    command: "cmatrix -b" # custom 才有：sh -c 跑的指令；空 = 未設，鎖定畫面顯示 ERROR
    bg: "#313244"          # custom 的顏色給 PIN 框與結束時的板子
    fg: "#f2b753"
...
35. （2026-09-25，使用者定案）第三種 saver **custom**：使用者自己輸入指令當保護程式的動畫，locku 管鎖、PIN、整合。程式在 locku 開的 pty 上、自己一個 process group，輸出直通真 tty，按鍵留在 locku；**不干涉生命週期**——不重啟、不讀畫面，鎖結束就 SIGKILL 整個 group；按鍵時暫停轉發、換成只有 PIN 框的 lock 程式，Esc / 逾時後恢復並送 SIGWINCH 讓程式重畫。程式結束就用 locku 的板子顯示字：exit 0 是 `COMPLETED`（不該跑完但跑完了），其他是 `ERROR`，large → small 退階，狀態列紅字寫原因，不重啟。否決：VT 模擬器路線、自動重啟、黑畫面加紅字。細節 5.5，驗收 `e2e/custom_lock.py`。
