...
func TestFitPicksTheScale(t *testing.T) {
	hm := saver.Clock{Time: saver.TimeHM, Date: saver.DateOff}
	full := saver.Clock{Time: saver.TimeHMS, Date: saver.DateYMD}
	cases := []struct {
		name       string
		s          saver.Clock
		cols, rows int
		...
		{"80x24 HH:MM", hm, 80, 23, 1, []string{"21:05"}},
		{"120x40 HH:MM", hm, 120, 39, 2, []string{"21:05"}},
		{"200x60 HH:MM", hm, 200, 59, 3, []string{"21:05"}},
		{"80x24 HH:MM:SS + YYYY-MM-DD degrades to HH:MM + MM-DD", full, 80, 23, 1, []string{"21:05", "09-24"}},
		{"40x12 HH:MM is plain text", hm, 40, 11, 0, []string{"21:05"}},
		{"40x12 full clock degrades all the way to plain HH:MM", full, 40, 11, 0, []string{"21:05"}},
	}
	for _, c := range cases {
		lines, k := fit(c.s, at, c.cols, c.rows)
...
func TestPaintCentresTheBlock(t *testing.T) {
	// 120×40: k = 2, "21:05" is 29 × 7 font pixels → 58 × 14 on a 60 × 39 board.
...
func TestRowsAreExactlyTheTerminalWide(t *testing.T) {
	...
	for _, cols := range []int{80, 81, 40, 7} {
		rows := 23
		lines, k := fit(saver.Clock{Time: saver.TimeHM, Date: saver.DateYMD}, at, cols, rows)
		var out []string
		if k >= 1 {
