...
func (m AppModel) View() string {
	...
	case m.width < narrowW && m.focus == panelSide:
		panels = panelFrame(m.width-2, m.sidebarBody(m.width-2, innerH), "[1] locku", "", true)
	case m.width < narrowW:
		panels = panelFrame(m.width-2, m.detailBody(m.width-2, innerH), m.detailTitle(), hint, true)
	default:
		w2 := m.width - sideW - 2
		panels = joinHorizontal(
			panelFrame(sideW-2, m.sidebarBody(sideW-2, innerH), "[1] locku", "", m.focus == panelSide),
			panelFrame(w2, m.detailBody(w2, innerH), m.detailTitle(), hint, m.focus == panelDetail))
	}
