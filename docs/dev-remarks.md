...
- **鎖定畫布不套 core key（K1、K5、K6、M1）。** 畫布上沒有 Space menu、沒有 `?`、沒有 footer：
  畫布唯一的動作是「任何鍵叫出 PIN prompt」，第一個鍵不算輸入；任何鍵若有別的意義，就會讓
...
三份設計文件每條決定就地標日期、被否決的做法也留著；`function.md` 的決定清單 41 條。
