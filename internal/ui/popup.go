...
// drawPopupBox is the shared popup frame (family form): the title sits in the
// top border, the hint in the bottom border, and the content is framed by one
// blank row top and bottom. One frame for every popup class means the user
...
// declared key, case and all. The bracket is generated from the same string
// (bracketHotkey), so what is on screen is the whole binding — nothing fires
// that the marking does not name, and nothing the marking names fails.
