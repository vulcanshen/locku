...
終端機的螢幕保護程式加 PIN 鎖。u-family 第五個成員（kbu / filu / sshu / webu / locku），依家族的 TUI 設計原則 [VTP](https://github.com/vulcanshen/thoughts/blob/main/tui-design/README.md) 設計；圖示是家族的方塊字 mark，[`docs/icon.svg`](docs/icon.svg)。
...
**Profiles** 是你設定好的、有名字的 saver（object）：new / duplicate / rename / delete 都在這裡，`●` 是啟用中的那個，`a` 把游標那個設為啟用（2026-09-25）。
...
`[1]` 的 saver：`n` new profile、`p` 預覽預設值；`[1]` 的 profile：`p` 預覽這個 profile、`a` 設為啟用、`D` duplicate、`r` rename、`X` delete；`[2]` profile / saver 上：`P` 預覽、`S` 存顏色草稿、`R` 丟掉（custom 只有 `P`）；
`[2]` preference 的 PIN 列：已設時 Enter 先驗目前的 PIN，再選 `New PIN` 或 `Remove PIN`（Remove 立即生效）。啟用哪個 profile 在 preference › profile 選，或在側欄的 profile 列按 `a`；`●` 標的就是它。`P` 只在 `[2]` 上預覽（profile / saver 的 `[2]` 是那一個、其他是啟用中的；`[1]` 上不作用，`p` 才是游標那列）、`q` 離開。
...
| 檔案 | 回答什麼 | 順序 |
|---|---|---|
| [`docs/function.md`](docs/function.md) | 為什麼不能在 pane 內攔 prefix、三種進入點同一契約、訊號表、狀態機、PIN 驗證、無 PIN 模式、`locku pin reset`、三種 saver（含 custom 的 pty 與疊框）、畫布渲染器與退階、CLI、config、Integration 怎麼寫檔（含 tmux / screen 的實測）、技術選型與平台、決定清單 41 條、MVP 驗收 | 1 |
...
發版走家族的流程：push 一個 `v*` tag，GitHub Actions 在兩個平台跑測試，goreleaser 打包並更新 Homebrew tap，release notes 是 `CHANGELOG.md` 對應的那一節。
