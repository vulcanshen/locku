...
func TestRevealTouchesOnlyWhatChanged(t *testing.T) {
	from := paint(faceTall, []string{"21 05"}, 2, 120, 39)
	to := paint(faceTall, []string{"21 06"}, 2, 120, 39)
	r := newReveal(from, to)
	if r == nil {
...
func TestRevealIsNilWhenNothingToDo(t *testing.T) {
	a := paint(faceTall, []string{"21 05"}, 2, 120, 39)
	if newReveal(a, a.clone()) != nil {
		t.Error("same board")
	}
	if newReveal(a, paint(faceTall, []string{"21 05"}, 1, 80, 23)) != nil {
