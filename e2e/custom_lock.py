...
check("the picture goes on under the prompt, the box painted over every frame", c2 > c1 and output(out).count(PROMPT) > 3)
os.write(fd, b"\x1b")
time.sleep(1.2)
c3 = output(out).count(MARK.encode())
check("Esc takes the box away and the program goes on", c3 > c2 and b"\x1b[?2026l" in output(out))
