...
| custom（2026-09-25） | `command`：使用者自己的指令，`sh -c` 跑，畫面由它畫；沒有 `bg` / `fg`（同日修訂，使用者）——PIN 框與板子用預設色 | 不經畫布：程式在 locku 開的 pty 上跑，輸出原樣轉到終端機；程式結束（它不該結束）就換成 locku 的板子照實寫 `EXIT <code>`（`EXIT` 金字，數字 0 綠、其他橘；沒跑起來是紅色的 `NONE`），見 5.5 | 無，由程式自己 |
...
### 5.5 custom：使用者自己的程式（2026-09-25）

使用者可以自己開發保護程式的動畫，讓 locku 管其他的——鎖、PIN、整合（使用者 2026-09-25 定案）。profile 只有一個 `command`，`sh -c` 跑（可帶參數與 pipe），不做任何 sanitize：是使用者自己機器上自己的指令，README 寫明。

- **誰擁有終端機**：locku。真 tty 由 locku 握著（raw、ISIG 關、alternate screen），程式跑在 locku 開的 **pty** 上、自己一個 process group：它以為自己有終端機（尺寸、SIGWINCH、curses 正常初始化），它的 termios、崩潰、亂送訊號都碰不到 locku 的 tty；能到真 tty 的只有它輸出的 bytes，locku 原樣轉發；離開時 locku 自己送 reset 序列收尾。按鍵永遠到 locku（否則 `q` 就把 cmatrix 關了、PIN 也收不到）。stderr 另接一條 pipe，留最後一行給狀態列。
- **不干涉生命週期**（使用者定案）：程式該無限迴圈；locku 不重啟、不讀它的畫面，只在鎖結束時對整個 process group 送 SIGKILL（`sh -c` 跟它底下的程式一起走）。尺寸變化轉給 pty，這是任何終端機都會做的事。
- **按鍵 → PIN**：不停止程式，只是暫停轉發它的輸出（讀掉丟掉，pty 不會塞住），離開 alt screen、還原 termios，跑一個只有 PIN 框的 lock 程式（同一套 prompt、cooldown、timeout，底是預設的暗色）；Esc 或逾時就結束那個程式、重新 raw + alt screen、恢復轉發、送一個 SIGWINCH 讓它重畫（curses 程式都會整頁重畫）；PIN 對了殺程式、結束。沒設 PIN 時第一個鍵就結束。tty 消失：殺程式、結束、tmux 旗不清。
- **程式結束**（使用者 2026-09-25 定案）：鎖不退，畫面換成 locku 現在的點陣板格式，字用 clock 的 3x7 字型，large → medium → small 依尺寸退階，放不下就純文字；狀態列紅字寫原因。板子照實寫 **`EXIT <code>`**，不另外取名（同日第三版，使用者：先是 COMPLETED / ERROR，再 DONE / ERROR——COMPLETED 字太多，最後決定直接寫退出碼最真實、也不用取名）：`EXIT` 四個字母金色（預設 fg），數字 0 綠、其他橘（peach；同日再修訂，使用者：紅留給 NONE）——板子因此第一次有兩種亮色，畫布多一個 accent；exit 0 → `EXIT 0`（跑完了，但不該跑完；狀態列 `custom saver exited 0`）；exit 非 0 → `EXIT 3`（狀態列 `custom saver: exit 3 · boom`，stderr 最後一行；找不到指令是 `EXIT 127`）；被訊號殺 → 跟 shell 一樣 128 + 訊號號碼，`EXIT 139`（狀態列 `killed: segmentation fault`）；沒填 command 或起不來 → `NONE` 整個紅字（`no command`）。都不重啟、PIN 照常。放不下最小尺寸時退成純文字，單色。
- **沒有 fg / bg**（同日修訂，使用者）：畫面是程式的，顏色設定沒意義，`[2]` 沒有色票列、沒有草稿與 Save / Reset，檔案裡也不寫；PIN 框與結束時的板子底用預設色（surface0），`EXIT` 金字、數字 0 綠 / 其他橘、`NONE` 紅。
- **preview**：設定畫面把終端機暫時交出去（Bubble Tea 的 exec），跑同一套，任意鍵殺掉回來；程式結束或沒填指令就回到畫面內、以板子上的字預覽，跟其他 saver 的預覽一樣。
- 否決：內建 VT 終端機模擬器把輸出 parse 成格子再畫（PIN 框能疊在動畫上、preview 不用交終端機；但多一個依賴、忠實度與效能都要驗）——選直通，忠實、可預期，失敗模式都在 locku 控制內；重啟三次再退階（使用者：不干涉生命週期）；黑畫面加紅字（使用者：用現在的板子顯示字）。
- 驗收：`make e2e` 的 `e2e/custom_lock.py` 在 pty 上跑真的 binary：無 PIN 任意鍵結束且程式被殺；設 PIN 後按鍵出 PIN 框、期間輸出停住、Esc 後輸出恢復、PIN 結束並殺程式；`exit 3`、`exit 0`、沒填指令三種板子與狀態列文字；本機有 cmatrix 就真的跑 cmatrix 並殺乾淨。
...
- preference：啟用中的 profile（`profile`）、show_status、`pin_prompt_timeout`、`wrong_pin_attempts` / `wrong_pin_attempt_cooldown`。設為啟用在這裡，側欄的 `●` 只顯示。每一列的意思：focus 在 preference 的 `[2]` 時按 `?`，help 就**只有**這幾列的說明（自動換行），沒有鍵的清單；其他地方的 `?` 是鍵（2026-09-25：原本列在每列下面，使用者要搬到 help，而且 `[2]` 上只要 preference 的說明）。tmux / screen 的 `[2]` 同理，只有 activate、config file path、lock-after-time（screen 是 idle）、bind-key（screen 是 bind）的說明，screen 再多一條講 LOCKPRG 住在 shell rc（2026-09-25）。
- Integration（2026-09-25，取代 `locku setup` 指令）：側欄第三個區塊，`tmux` 與 `screen` 各一項，各自有 `config file path`（要寫的檔案；同日中午叫 `conf`）與閒置幾秒自動鎖，後者**用工具自己的設定名稱**：tmux 是 `lock-after-time`、screen 是 `idle`（2026-09-25 修訂，使用者：tmux 就用 tmux 的設定名稱；同日早上兩個都叫 `idle_lock`，config key 一併改、舊 key 自動轉），各自獨立、不再共用一個值；預設 300，0 關閉；tmux 再多一列 `bind-key`（2026-09-25，使用者：prefix shortcut）：prefix 之後按哪個鍵就鎖整台，照 tmux 的寫法（`l`、`C-l`、`F12`），Enter 開 input 框；有值就在區塊多寫一行 `bind-key <鍵> lock-server`、有 server 在跑就即時 bind；清空就是不綁，那一行不寫、server 上原本綁的鍵 unbind；含空白或 `#` 拒收。screen 同一列叫 `bind`（2026-09-25，使用者定案：原理照 tmux 那邊的作法，名字用 screen 自己的）：C-a 之後按哪個鍵就鎖，照 screen 的 `bind` 寫法（`l`、`^L`），有值就在區塊多寫一行 `bind <鍵> lockscreen`、有 session 在跑就 `screen -X bind` 即時綁；空就不綁——screen 內建的 `C-a x` 本來就是 lockscreen，`?` 說明要講清楚；同樣拒收空白與 `#`。screen 沒有 `lock` 那列：每個 screen 是自己一個 process、LOCKPRG 跟著 shell 走，沒有 server / session 這種範圍可選，不硬造一個。tmux 還有 `lock`（2026-09-25，使用者定案；研究在 `.local/studies/lock.md`）：鎖的**範圍**——`lock-server`（預設）鎖整台 server，鎖著時 attach 任何 session 都被鎖；`lock-session` 只鎖觸發的那個 session：它的 client、以及之後 attach 它的人，別的 session 照常。值用 tmux 的指令名，因為 alias 與 bind-key 最後跑的就是它，但這列與 `?` 說明講的是範圍、不講觸發方式（同日修訂，使用者：原本的說明扯到 bind-key，看不懂、會誤導）；options popup 二選一，同樣在分隔線下方。`[2]` 的版面（2026-09-25 傍晚定案，使用者：回歸 property / value）：表頭下第一列 `activate`——`on` / `off`，就是區塊在不在檔案裡，每次畫都讀檔；第二列 `config file path`；一條分隔線；然後工具自己的 key（閒置鎖、tmux 的 lock 與 bind-key、screen 的 bind），分隔線區隔「locku 的設定」與「寫進工具設定檔的 key」。`activate` 的 Enter 跳 confirm 才執行：on 把區塊寫進檔案（tmux 有 server 在跑就即時套用）、off 拿掉；路徑沒填時 disabled 並說明。同日的前幾版都否決：早上一列 dim 的 `tool` 名稱加 `block` `in the file`（看不懂）、中午一列 `status`、下午狀態放進標題膠囊加 `[S]` / `[X]` 熱鍵（畫面上看不到，使用者無法直觀知道）、再來底部分隔線下一顆 Install / Uninstall 按鈕（風格不對）。**開一次，之後隨設即得**：on 時任何一列改動就直接重寫區塊、tmux 即時套到 server、screen 即時送進跑著的 session（2026-09-25），config file path 改路徑就把區塊從舊檔搬到新檔、清空就拿掉；off 就只寫 config.yaml，activate 仍由使用者開。兩個純方案各有硬傷才折衷：純隨設即得會在 conf 打錯時生檔、Remove 只能等於清 conf、不能先填好再裝；純按鈕會 stale——改了 lock-after-time 檔案裡還是舊值。`conf` 是 locku 唯二的自由輸入，用 webu 的 input 作法：框裡先 dim 顯示一個**提議**——目前值，沒有就是慣例的 `~/.tmux.conf` / `~/.screenrc`——Tab 接手編輯、Backspace 拒絕、打字就從頭打；Enter 照打的存，沒碰提議就 Enter 不改。
...
（2026-09-25：從 CLI `locku setup [-d]` 搬進 TUI 的 Integration 區塊，同日從 `[S]` / `[X]` 熱鍵、再從底部的 Install 按鈕，改成 `[2]` 第一列 `activate` 的開關，Enter 後 confirm；下文的「setup」指寫入這個動作。）只寫受管區塊，區塊外一個字都不動；再寫一次就是替換區塊，冪等——裝著的時候每改一列就這樣重寫一次。

| 目標 | 檔案 | 區塊內容 |
|---|---|---|
| tmux | Integration › tmux 的 `conf`（使用者輸入，`~/` 可用，不存在就建；沒設 activate 就 disabled、說先填 config file path，不猜） | `set -gF lock-command "<locku 的絕對路徑> lock -S '#{socket_path}'"`、`set -g lock-after-time <tmux 的 lock-after-time>`、`set -s "command-alias[90]" "locku=lock-server"`、`set-hook -g "client-attached[90]" "if -F \"#{@locked}\" lock-client"`、`set-hook -g "client-session-changed[90]" "if -F \"#{@locked}\" lock-client"`；`lock` 是 lock-session 時 alias 是 `locku=lock-session`、再一行 `set-hook -g "session-created[90]" "set -F lock-command \"<絕對路徑> lock -S '#{socket_path}' -t '#{session_id}'\""`；Integration › tmux › bind-key 有填就再一行 `bind-key <鍵> <lock>`；每一行尾巴都有 `# locku` 註解 |
| screen | Integration › screen 的 `conf`（同上），加上 shell rc：`$SHELL` 是 zsh 寫 `~/.zshrc`、bash 寫 `~/.bashrc`、fish 寫 `~/.config/fish/config.fish`、其他寫 `~/.profile` | `.screenrc`：`idle <screen 的 idle> lockscreen`；Integration › screen › bind 有填就再一行 `bind <鍵> lockscreen`（2026-09-25）；shell rc：`export LOCKPRG=<絕對路徑>`（fish 是 `set -gx LOCKPRG <絕對路徑>`）；每一行尾巴都有 `# locku` 註解 |
...
- 路徑由使用者在 Integration 各項的 `conf` 輸入（2026-09-24 起在 preference，2026-09-25 搬來）：沒設時 `activate` 是 disabled 並說 `set the config file path first`，什麼都不寫（screen 連 shell rc 也不寫）；原本「`~/.tmux.conf` 不在就找 `~/.config/tmux/tmux.conf`」的猜法拿掉。相對路徑拒收，它會落在程式剛好執行的目錄。
- tmux 有 server 在跑時同時即時套用同樣五條（`tmux set -gF lock-command …` 等），有 bind-key 就多 `bind-key <鍵> <lock>`；lock-session 再多 session-created hook，並替每個既有 session 設它自己的 lock-command；換 lock 時先清掉全域與每個 session 的 `@locked`、拿掉 hook（研究 §5：換檔後殘留的全域旗會讓所有 session 看似被鎖）；activate off 時反向 `set -gu` / `set -su` / `set-hook -gu` 一條對一條拿掉、綁過的鍵 `unbind-key`、每個 session 的 lock-command 與旗、再清 `@locked`。Setup 時檔案裡原本綁的鍵跟這次不同（改了或清空），先 `unbind-key` 舊的再套用（2026-09-25）；綁過與否看的是檔案裡區塊的 `bind-key` 行，不另外記。unbind 之後那個鍵 tmux 內建的功能（例如 `l` 的 last-window）要 server 重啟才回來。沒有 tmux 或沒有 server 就跳過並說明。結果以 toast 一行回報（原本印在終端機的幾行，以 ` · ` 接起來）。
...
- 執行後印出改了哪個檔、有沒有即時套用（screen：套到幾個跑著的 session）、還需要做什麼（screen：新開 shell；已在跑的 session detach 後從新 shell 重新 attach，LOCKPRG 才是 locku）。
- 不備份。移除就是 `[2]` 的 `activate` 關掉（confirm 後；2026-09-25 下午是 Uninstall 按鈕、早上是 `[X]` 熱鍵，前一天是 `locku setup -d`）：把受管區塊從檔案拿掉、有 server 在跑就一併拿掉；區塊前面補的空行也一起拿掉，其餘一個字不動；沒有區塊就說沒有；檔案不存在不會生出來。screen 的 Remove 同時清 `.screenrc` 與 shell rc 的區塊，跑著的 session 也 `-X idle 0`、綁過的鍵 `-X bind <鍵>` 解掉（2026-09-25；解掉之後那個鍵 screen 內建的功能——例如 `l` 的 redisplay——要 session 重開才回來，跟 tmux 一樣）。`[2]` 的 `activate` 列每次畫都讀一次檔案，說區塊在不在。
## 7. 設定與儲存
...
profiles:
  ...
  - name: matrix
    ...
    command: "cmatrix -b" # custom 才有：sh -c 跑的指令；空 = 未設，鎖定畫面顯示 ERROR
...
- 讀取失敗的處理見 4.3。
- 閒置多久自動鎖由各工具自己的那一列決定——tmux 的 `lock-after-time`、screen 的 `idle`，名字就是工具自己的（2026-09-25 拆開並改名，原本一個共用的 `idle_lock`；更早是寫死 300 在區塊裡），activate on 時原樣寫進去、一改就重寫；locku 自己不計時。

## 8. 安裝與整合（README 要交付的內容）

順序不限：不設定就是純螢幕保護，任何鍵解鎖；要密碼再執行 `locku` 設 PIN。

在 `locku` 側欄 Integration › tmux 的 `[2]` 把 `activate` 打開（confirm 後）直接寫進設定檔，做法見 6.2；開著時改任何一列就直接重寫。以下是它寫的內容，手動設定也是同一份：
...
最後那行只在 Integration › tmux › bind-key 有填時才寫（這裡填的是 `l`）；`lock` 選 `lock-session` 時 alias 與 bind-key 指向 `lock-session`，並多一行 `set-hook -g "session-created[90]" …`（見 6.2）。鎖：`prefix :` 打 `locku`（或 shell 的 `tmux locku`，填了 bind-key 就 `prefix l`）整台的 client 全鎖，閒置 300 秒的畫面也鎖；鎖著的時候誰 attach 哪個 session 都會看到保護程式。移除：`activate` 關掉。

screen，寫進 Integration › screen › conf（慣例 `~/.screenrc`），同一套區塊標記與 `# locku` 註解（2026-09-25）：
...
最後那行只在 Integration › screen › bind 有填時才寫（這裡填的是 `l`）；`C-a x` 是 screen 內建的 lockscreen，不填也鎖。加上 shell rc（`~/.zshrc` 或 `~/.bashrc`；fish 用 `set -gx`），因為只有 attacher 的環境會被 lock 讀到（6.2）：
...
- Integration › tmux 的 activate 開著寫兩次，設定檔內容相同；有 server 時 `tmux show -g lock-command` 立即是 `<絕對路徑> lock -S '<socket>'`；activate 關掉後區塊消失、server 上五條（有 bind-key 是六條）都拿掉（setup 套件測試；TUI 的 activate 在 app 測試與 `make e2e` 裡真的按；on 時改 lock-after-time、清 bind-key、搬 config file path 都在 app 測試裡驗檔案）。
- 鎖著的時候誰進來都被鎖（2026-09-24，`make e2e` 在真的 tmux 3.7c 上跑）：`locku` 後 client A 看到點陣板、全域標 `@locked`；B 此時 attach 同一個 session、E attach 另一個 session，都看到點陣板；A 解鎖後 `@locked` 消失、B 與 E 仍鎖著直到各自解鎖；之後 attach 的 C 不被鎖；設定畫面填的 `bind-key l` 寫進區塊、server 上有這個 bind（2026-09-25）；A 這次由 `prefix l` 鎖住，鎖定中 A 的終端機死掉，`@locked` 留著，接著 attach 的 D 被鎖、D 解鎖才清掉；activate 關掉後區塊消失；lock-session 一輪（2026-09-25）：畫面上選 lock-session 立刻重寫檔案（alias、bind-key、session-created hook），server 上每個 session 的 lock-command 帶自己的 id，`locku` 後旗只在那個 session、全域沒有，B attach 同 session 被鎖、E attach 另一個 session 不受影響，A 解鎖清掉 session 的旗。
