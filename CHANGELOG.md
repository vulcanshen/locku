...
## [Unreleased]
### Added
- `locku lock`: the terminal as one LED board — every cell a Nerd Font square, dark in the saver's `bg`, lit in its `fg` — spelling the clock in a 5 × 7 pixel font at the saver's `size` (small, medium, large: one font pixel is 1, 2 or 3 cells square); what does not fit sheds the year, the seconds, then the date before the size steps down. A change lands as a shuffled reveal of only the pixels that differ.
